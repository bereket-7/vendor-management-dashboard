...
const DEMO_SUBSCRIBER_IDS: Record<string, { external: string; alternate: string }> =
	{
		"099898779": { external: "87654321", alternate: "64987572" },
		"099876543": { external: "87654324", alternate: "64981234" },
	};

const DEMO_DEPENDENT_IDS: Record<string, { external: string; alternate: string }> =
	{
...
const DEMO_FAMILY_1_SUBSCRIBER: Omit<DemoMemberOverlay, "memberId" | "externalId" | "alternateId"> =
	{
		firstName: "Abebe",
		lastName: "Kebede",
		dob: "1990-03-15",
...
const DEMO_FAMILY_1_DEPENDENT: Omit<DemoMemberOverlay, "memberId" | "externalId" | "alternateId"> =
	{
		firstName: "Bethel",
		lastName: "Abebe",
		dob: "2015-06-03",
...
const DEMO_FAMILY_2_SUBSCRIBER: Omit<DemoMemberOverlay, "memberId" | "externalId" | "alternateId"> =
	{
		firstName: "Tigist",
...
export function resolveDemoCardholderId(cardholderId: string): string {
	...
	return LEGACY_CARDHOLDER_TO_DEMO[raw] ?? LEGACY_CARDHOLDER_TO_DEMO[stripped] ?? stripped;
}
...
export function getDemoMemberOverlay(input: {
	...
	let template: Omit<DemoMemberOverlay, "memberId" | "externalId" | "alternateId"> | null =
		null;

	if (ids.cardholderId === "099898779") {
		template = personCode === "02" ? DEMO_FAMILY_1_DEPENDENT : DEMO_FAMILY_1_SUBSCRIBER;
	...
	return {
		...
		newtechMemberId: fixNewtechId(input.newtechMemberId, DEMO_NEWTECH_MEMBER_ID),
		newtechFamilyId: fixNewtechId(input.newtechFamilyId, DEMO_NEWTECH_FAMILY_ID),
	};
}
