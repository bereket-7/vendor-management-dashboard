...
export const vmsKeys = {
	...
	document: (id: string) => [...vmsKeys.all, "document", id] as const,
...
export function useDocument(id: string | undefined) {
	const q = useQuery({
		queryKey: vmsKeys.document(id ?? ""),
		queryFn: () => vmsApi.getDocument(id!),
		enabled: !!id,
	});
	return { document: q.data ?? null, isLoading: q.isLoading, error: q.error };
}
...
export function useUpdateDocumentMutation() {
	...
	return useMutation({
		...
		}) => vmsApi.updateDocument(id, patch),
		onSuccess: invalidate,
	});
}
